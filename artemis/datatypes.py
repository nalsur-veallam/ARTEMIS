...
class MAP:
    ...
    def read_data(self, data, filename=None) -> None:
        try:
            ...
            self.errors.append(
                ValueError(f"Can't get data from file {filename} by 'map' key.")
            )
        ...
        try:
            ...
            self.errors.append(
                ValueError(f"Can't get data from file {filename} by 'NResidues' key.")
            )
        ...
        try:
            ...
            self.errors.append(
                ValueError(f"Can't get data from file {filename} by 'names' key.")
            )
        ...
        try:
            ...
            self.errors.append(
                ValueError(
                    f"Can't get data from file {filename} by 'real_numbers' key."
                )
            )
    ...
    def check(self) -> None:
        if len(self.names) != self.NResidues:
            self.errors.append(
                ValueError(
                    "The received data is broken: the size of the names array does not match the number of residues."
                )
            )
        ...
        if self.map_.shape[0] != self.NResidues or self.map_.shape[1] != self.NResidues:
            self.errors.append(
                ValueError(
                    "The received data is broken: the size of the MIE matrix does not match the number of residues."
                )
            )
        ...
        if len(self.real_numbers) != self.NResidues:
            self.errors.append(
                ValueError(
                    "The received data is broken: the size of the real_numbers array does not match the number of residues."
                )
            )
    ...
    def __init__(self, filename=None, data=None):
        ...
        if filename is not None:
            try:
                ...
                self.errors.append(
                    ValueError("Can't read data from file", filename, ".")
                )
...
class ALLOSTERY:
    ...
    def read_data(self, data, filename=None) -> None:
        ...
        try:
            ...
            self.errors.append(
                ValueError(f"Can't get data from file {filename} by 'active_site' key.")
            )
        ...
        try:
            ...
            print(
                f"Caution: Can't get data from file {filename} by 'allosteric_site' key. Continues without this data."
            )
            self.allosteric_site = None
        ...
        try:
            ...
            print(
                f"Caution: Can't get data from file {filename} by 'intensity' key. Continues without this data."
            )
            self.intensity = None
    ...
    def check(self) -> None:
        ...
        if (
            max(self.active_site) > self.NResidues
            or min(self.active_site) < 1
            or len(self.active_site) == 0
        ):
            self.errors.append(
                ValueError("The received data is broken: incorrect active site format.")
            )

        if self.allosteric_site is not None and (
            max(self.allosteric_site) > self.NResidues
            or min(self.allosteric_site) < 1
            or len(self.allosteric_site) == 0
        ):
            self.errors.append(
                ValueError(
                    "The received data is broken: incorrect allosteric site format."
                )
            )

        if self.intensity is not None and min(self.intensity) < 0:
            self.errors.append(
                ValueError("The received data is broken: incorrect intensity data.")
            )
    ...
    def from_map_and_groups(
        self,
        Map,
        Groups,
        active_site_key="active_site",
        allosteric_site_key="allosteric_site",
    ) -> None:
        if not Map.exist() or not Map.is_ok():
            self.errors.append(ValueError("The received Map object is broken."))
        if not Groups.exist() or not Groups.is_ok():
            self.errors.append(ValueError("The received Groups object is broken."))
        ...
        if active_site_key in Groups.data.keys():
            ...
            self.errors.append(
                ValueError(
                    f"The received data is broken: can't find active site group by {active_site_key} key."
                )
            )
...
class CLUSTERS:
    ...
    def read_data(self, data, filename=None) -> None:
        ...
        try:
            ...
            print(
                f"Caution: Can't get data from file {filename} by 'active_site' key. Continues without this data."
            )
            self.active_site = None
        ...
        try:
            ...
            print(
                f"Caution: Can't get data from file {filename} by 'reference_group' key. Continues without this data."
            )
            self.reference_group = None
        ...
        try:
            ...
            print(
                f"Caution: Can't get data from file {filename} by 'restriction_group' key. Continues without this data."
            )
            self.restriction_group = None
        ...
        try:
            ...
            print(
                f"Caution: Can't get data from file {filename} by 'clustering_labels' key. Continues without this data."
            )
            self.clustering_labels = None
        ...
        try:
            ...
            print(
                f"Caution: Can't get data from file {filename} by 'NClusters' key. Continues without this data."
            )
    ...
    def check(self) -> None:
        ...
        if self.restriction_group is not None and (
            max(self.restriction_group) > self.NResidues
            or min(self.restriction_group) < 1
            or len(self.restriction_group) == 0
        ):
            self.errors.append(
                ValueError(
                    "The received data is broken: incorrect restriction group format."
                )
            )

        if self.reference_group is not None and (
            max(self.reference_group) > self.NResidues
            or min(self.reference_group) < 1
            or len(self.reference_group) == 0
        ):
            self.errors.append(
                ValueError(
                    "The received data is broken: incorrect reference_group format."
                )
            )

        if self.NClusters is not None and self.NClusters <= 0:
            self.errors.append(
                ValueError("The received data is broken: incorrect NClusters format.")
            )

        if (
            self.clustering_labels is not None
            and len(self.clustering_labels) != self.NResidues
        ):
            self.errors.append(
                ValueError(
                    "The received data is broken: the size of the clustering_labels array does not match the number of residues."
                )
            )

        if self.active_site is not None and (
            max(self.active_site) > self.NResidues
            or min(self.active_site) < 1
            or len(self.active_site) == 0
        ):
    ...
    def from_map_and_groups(
        ...
        Map,
        Groups,
        reference_group_key="reference_group",
        restriction_group_key="restriction_group",
        active_site_key="active_site",
        allosteric_site_key="allosteric_site",
    ) -> None:
        if not Map.exist() or not Map.is_ok():
