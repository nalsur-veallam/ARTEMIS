...
if not ("-strc" in sys.argv and "-asn" in sys.argv and "-f_act" in sys.argv and "-n" in sys.argv and len(sys.argv) >= 9):
    print("USAGE:\n"+sys.argv[0]+" -f_act active_site.json -n name -asn active_site_name -f_all allosteric_site.json -allsn allosteric_site_name -strc sctructure.pdb(.gro ...) -filt -sasa_filt -noseq num_of_res(default 0) -top top\n")
    exit()
...
if top is None:
    ...
    try:
        cmd.save(out_path + "_intensity_top"+str(top)+".pse")
        print("File",out_path + "_intensity_top"+str(top)+".pse created\n")
    except:
        print("Error writing file",out_path + '_intensity_top'+str(top)+'.pse\n')
...
for i in tqdm(range(NResidues)):
    if i + 1 in active_site:
        intensity.append(-1)
    elif i + 1 not in all_site:
        mi_act = 0
        mi_all = 0
        entr = map_[i][i]
        for resid in active_site:
            if np.abs(resid - 1 - i) >= noseq:
                mi_act += map_[resid - 1][i]
        for resid in all_site:
            if np.abs(resid - 1 - i) >= noseq:
                mi_all += map_[resid - 1][i]
...
try:
    cmd.save(out_path + "_crit.pse")
    print("File",out_path + "_crit.pse created\n")
except:
    print("Error writing file",out_path + '_crit.pse\n')
