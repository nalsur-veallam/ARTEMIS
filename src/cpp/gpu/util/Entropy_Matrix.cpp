// The class code for handling an Entropy_Matrix from a .par file of the
...
Entropy_Matrix::Entropy_Matrix(char const *infileInput) {
  ...
  try {
    ...
    My_Error my_error((string("ERROR WHILE READING THE HEADER OF THE FILE ") +
                       string(infileInput) + string("! ABORTING."))
                          .c_str());
    throw my_error;
  }
...
Entropy_Matrix::Entropy_Matrix(unsigned int nAtoms) {
  ...
  if (!((bondsEntropy1D != NULL) && (anglesEntropy1D != NULL) &&
        ...
        (baEntropy != NULL) && (bdEntropy != NULL) && (aaEntropy != NULL) &&
        (adEntropy != NULL) && (ddEntropy != NULL))) {
    My_Error my_error(
        "ERROR: ALLOCATION FOR PAR FILE BODY FAILED. MORE MEMORY NEEDED?");
    throw my_error;
  }

  double_prec = -1; // set the precision of the .bat file which underlies this
                    // .par file (Entropy_Matrix) to "unknown"
  numFrames = 0; // initialize the number of frames of the underlying .bat
                 // trajectory to 0, indicating unknown
  ...
  for (int i = 0; i < 5; i++) {
    dummyintvec.push_back(0);
  }
  for (unsigned int i = 0; i < nDihedrals; i++) {
    dihedrals_top.push_back(dummyintvec);
  }
...
Entropy_Matrix::Entropy_Matrix(char const *bat_file, PRECISION *storage,
  ...
  if (!infile.good()) {
    My_Error my_error((string("ERROR OPENING FILE ") + string(bat_file) +
                       string("! ABORTING.\n"))
                          .c_str());
    throw my_error;
  }
  ...
  try {
    ...
    My_Error my_error((string("ERROR WHILE READING THE HEADER OF THE FILE ") +
                       string(bat_file) + string("! ABORTING."))
                          .c_str());
    throw my_error;
  }
...
void Entropy_Matrix::write(char const *outfileInput) {
  ...
  if (!outfile.good()) {
    My_Error my_error((string("ERROR OPENING FILE ") + string(outfileInput) +
                       string("! ABORTING.\n"))
                          .c_str());
    throw my_error;
  }
  ...
  try {
    ...
    My_Error my_error((string("ERROR WHILE WRITING THE HEADER OF THE FILE ") +
                       string(outfileInput) + string("! ABORTING."))
                          .c_str());
    throw my_error;
  }
...
PRECISION Entropy_Matrix::get2DEntropy(int type1, int type2, unsigned int index1,
  ...
  if ((index1 >= 0) && (index2 >= 0)) {
    if ((type1 == TYPE_B) && (type2 == TYPE_B) && (index1 != index2) &&
        (index1 < nBonds) &&
        (index2 < nBonds)) { // for 2D entropy of two different bonds
      smaller = index1 < index2 ? index1 : index2;
      bigger = index1 < index2 ? index2 : index1;
      index = (nBonds - smaller) * (nBonds - smaller - 1) / 2 + smaller -
              bigger; // the 2D-entropies bonds-bonds (also angles-angles and
                      // dihedrals-dihedrals) were stored in reverse order, as
                      // documented in "Parent.cpp"
      return bbEntropy[index];
    }
    if ((type1 == TYPE_B) && (type2 == TYPE_A) && (index1 < nBonds) &&
        (index2 < nAngles)) { // for 2D entropy of a bond and an angle
      return baEntropy[index1 * nAngles + index2];
    }
    if ((type1 == TYPE_A) && (type2 == TYPE_B) && (index1 < nAngles) &&
        (index2 < nBonds)) { // for 2D entropy of an angle and a bond
      return baEntropy[index2 * nAngles + index1];
    }
    if ((type1 == TYPE_B) && (type2 == TYPE_D) && (index1 < nBonds) &&
        (index2 < nDihedrals)) { // for 2D entropy of a bond and a dihedral
      return bdEntropy[index1 * nDihedrals + index2];
    }
    if ((type1 == TYPE_D) && (type2 == TYPE_B) && (index1 < nDihedrals) &&
        (index2 < nBonds)) { // for 2D entropy of a dihedral and a bond
      return bdEntropy[index2 * nDihedrals + index1];
    }
    if ((type1 == TYPE_A) && (type2 == TYPE_A) && (index1 != index2) &&
        (index1 < nAngles) &&
        (index2 < nAngles)) { // for 2D entropy of two different angles
      smaller = index1 < index2 ? index1 : index2;
      bigger = index1 < index2 ? index2 : index1;
      index = (nAngles - smaller) * (nAngles - smaller - 1) / 2 + smaller -
              bigger; // the 2D-entropies angles-angles were stored in reverse
                      // order, as documented in "Parent.cpp"
      return aaEntropy[index];
    }
    if ((type1 == TYPE_A) && (type2 == TYPE_D) && (index1 < nAngles) &&
        (index2 < nDihedrals)) { // for 2D entropy of a bond and an angle
      return adEntropy[index1 * nDihedrals + index2];
    }
    if ((type1 == TYPE_D) && (type2 == TYPE_A) && (index1 < nDihedrals) &&
        (index2 < nAngles)) { // for 2D entropy of an angle and a bond
      return adEntropy[index2 * nDihedrals + index1];
    }
    if ((type1 == TYPE_D) && (type2 == TYPE_D) && (index1 != index2) &&
        (index1 < nDihedrals) &&
        (index2 < nDihedrals)) { // for 2D entropy of two different dihedrals
      smaller = index1 < index2 ? index1 : index2;
      bigger = index1 < index2 ? index2 : index1;
      index = (nDihedrals - smaller) * (nDihedrals - smaller - 1) / 2 +
              smaller -
              bigger; // the 2D-entropies dihedrals-dihedrals were stored in
                      // reverse order, as documented in "Parent.cpp"
...
PRECISION Entropy_Matrix::getMutual(int type1, int type2, unsigned int index1,
  ...
  if ((index1 >= 0) && (index2 >= 0)) {
    if ((type1 == TYPE_B) && (type2 == TYPE_B) && (index1 != index2) &&
        (index1 < nBonds) &&
        (index2 <
         nBonds)) { // for mutual information between two different bonds
      smaller = index1 < index2 ? index1 : index2;
      bigger = index1 < index2 ? index2 : index1;
      index = (nBonds - smaller) * (nBonds - smaller - 1) / 2 + smaller -
              bigger; // the 2D-entropies bonds-bonds (also angles-angles and
                      // dihedrals-dihedrals) were stored in reverse order, as
                      // documented in "Parent.cpp"
      return bondsEntropy1D[smaller] + bondsEntropy1D[bigger] -
             bbEntropy[index];
    }
    if ((type1 == TYPE_B) && (type2 == TYPE_A) && (index1 < nBonds) &&
        (index2 <
         nAngles)) { // for mutual information between a bond and an angle
      return bondsEntropy1D[index1] + anglesEntropy1D[index2] -
             baEntropy[index1 * nAngles + index2];
    }
    if ((type1 == TYPE_A) && (type2 == TYPE_B) && (index1 < nAngles) &&
        (index2 <
         nBonds)) { // for mutual information between an angle and a bond
      return bondsEntropy1D[index2] + anglesEntropy1D[index1] -
             baEntropy[index2 * nAngles + index1];
    }
    if ((type1 == TYPE_B) && (type2 == TYPE_D) && (index1 < nBonds) &&
        (index2 <
         nDihedrals)) { // for mutual information between a bond and a dihedral
      return bondsEntropy1D[index1] + dihedralsEntropy1D[index2] -
             bdEntropy[index1 * nDihedrals + index2];
    }
    if ((type1 == TYPE_D) && (type2 == TYPE_B) && (index1 < nDihedrals) &&
        (index2 <
         nBonds)) { // for mutual information between a dihedral and a bond
      return bondsEntropy1D[index2] + dihedralsEntropy1D[index1] -
             bdEntropy[index2 * nDihedrals + index1];
    }
    if ((type1 == TYPE_A) && (type2 == TYPE_A) && (index1 != index2) &&
        (index1 < nAngles) &&
        (index2 <
         nAngles)) { // for mutual information between two different angles
      smaller = index1 < index2 ? index1 : index2;
      bigger = index1 < index2 ? index2 : index1;
      index = (nAngles - smaller) * (nAngles - smaller - 1) / 2 + smaller -
              bigger; // the 2D-entropies angles-angles were stored in reverse
                      // order, as documented in "Parent.cpp"
      return anglesEntropy1D[smaller] + anglesEntropy1D[bigger] -
             aaEntropy[index];
    }
    if ((type1 == TYPE_A) && (type2 == TYPE_D) && (index1 < nAngles) &&
        (index2 <
         nDihedrals)) { // for mutual information between a bond and an angle
      return anglesEntropy1D[index1] + dihedralsEntropy1D[index2] -
             adEntropy[index1 * nDihedrals + index2];
    }
    if ((type1 == TYPE_D) && (type2 == TYPE_A) && (index1 < nDihedrals) &&
        (index2 <
         nAngles)) { // for mutual information between an angle and a bond
      return anglesEntropy1D[index2] + dihedralsEntropy1D[index1] -
             adEntropy[index2 * nDihedrals + index1];
    }
    if ((type1 == TYPE_D) && (type2 == TYPE_D) && (index1 != index2) &&
        (index1 < nDihedrals) &&
        (index2 < nDihedrals)) { // for mutual information between two different
                                 // dihedrals
      smaller = index1 < index2 ? index1 : index2;
      bigger = index1 < index2 ? index2 : index1;
      index = (nDihedrals - smaller) * (nDihedrals - smaller - 1) / 2 +
              ...
              bigger; // the 2D-entropies dihedrals-dihedrals were stored in
                      // reverse order, as documented in "Parent.cpp"
      return dihedralsEntropy1D[smaller] + dihedralsEntropy1D[bigger] -
             ddEntropy[index];
    }
  }
  return 0;
...
void Entropy_Matrix::set2DEntropy(int type1, int type2, unsigned int index1,
  ...
  if ((index1 >= 0) && (index2 >= 0)) {
    if ((type1 == TYPE_B) && (type2 == TYPE_B) && (index1 != index2) &&
      ...
      index = (nBonds - smaller) * (nBonds - smaller - 1) / 2 + smaller -
              bigger; // the 2D-entropies bonds-bonds (also angles-angles and
                      // dihedrals-dihedrals) are stored in reverse order, as
                      // documented in "Parent.cpp"
    ...
    if ((type1 == TYPE_A) && (type2 == TYPE_A) && (index1 != index2) &&
      ...
      bigger = index1 < index2 ? index2 : index1;
      index = (nAngles - smaller) * (nAngles - smaller - 1) / 2 + smaller -
              bigger; // the 2D-entropies angles-angles are stored in reverse
                      // order, as documented in "Parent.cpp"
    ...
    if ((type1 == TYPE_D) && (type2 == TYPE_D) && (index1 != index2) &&
      ...
      index = (nDihedrals - smaller) * (nDihedrals - smaller - 1) / 2 +
              smaller -
              bigger; // the 2D-entropies dihedrals-dihedrals are stored in
                      // reverse order, as documented in "Parent.cpp"
    ...
    if ((type1 == TYPE_B) && (type2 == TYPE_B) && (index1 != index2) &&
      ...
      index = (nBonds - smaller) * (nBonds - smaller - 1) / 2 + smaller -
              bigger; // the 2D-entropies bonds-bonds (also angles-angles and
                      ...
                      // documented in "Parent.cpp"
      bbEntropy[index] =
          bondsEntropy1D[smaller] + bondsEntropy1D[bigger] - value;
    }
    if ((type1 == TYPE_B) && (type2 == TYPE_A) && (index1 < nBonds) &&
        (index2 <
         nAngles)) { // for mutual information between a bond and an angle
      baEntropy[index1 * nAngles + index2] =
          bondsEntropy1D[index1] + anglesEntropy1D[index2] - value;
    }
    if ((type1 == TYPE_A) && (type2 == TYPE_B) && (index1 < nAngles) &&
        (index2 <
         nBonds)) { // for mutual information between an angle and a bond
      baEntropy[index2 * nAngles + index1] =
          bondsEntropy1D[index2] + anglesEntropy1D[index1] - value;
    }
    if ((type1 == TYPE_B) && (type2 == TYPE_D) && (index1 < nBonds) &&
        (index2 <
         nDihedrals)) { // for mutual information between a bond and a dihedral
      bdEntropy[index1 * nDihedrals + index2] =
          bondsEntropy1D[index1] + dihedralsEntropy1D[index2] - value;
    }
    if ((type1 == TYPE_D) && (type2 == TYPE_B) && (index1 < nDihedrals) &&
        (index2 <
         nBonds)) { // for mutual information between a dihedral and a bond
      bdEntropy[index2 * nDihedrals + index1] =
          bondsEntropy1D[index2] + dihedralsEntropy1D[index1] - value;
    }
    if ((type1 == TYPE_A) && (type2 == TYPE_A) && (index1 != index2) &&
        (index1 < nAngles) &&
        (index2 <
      ...
      index = (nAngles - smaller) * (nAngles - smaller - 1) / 2 + smaller -
              bigger; // the 2D-entropies angles-angles were stored in reverse
                      // order, as documented in "Parent.cpp"
      aaEntropy[index] =
          anglesEntropy1D[smaller] + anglesEntropy1D[bigger] - value;
    }
    if ((type1 == TYPE_A) && (type2 == TYPE_D) && (index1 < nAngles) &&
        (index2 <
         nDihedrals)) { // for mutual information between a bond and an angle
      adEntropy[index1 * nDihedrals + index2] =
          anglesEntropy1D[index1] + dihedralsEntropy1D[index2] - value;
    }
    if ((type1 == TYPE_D) && (type2 == TYPE_A) && (index1 < nDihedrals) &&
        (index2 <
         nAngles)) { // for mutual information between an angle and a bond
      adEntropy[index2 * nDihedrals + index1] =
          anglesEntropy1D[index2] + dihedralsEntropy1D[index1] - value;
    }
    if ((type1 == TYPE_D) && (type2 == TYPE_D) && (index1 != index2) &&
        (index1 < nDihedrals) &&
        (index2 < nDihedrals)) { // for mutual information between two different
      ...
      index = (nDihedrals - smaller) * (nDihedrals - smaller - 1) / 2 +
              ...
              bigger; // the 2D-entropies dihedrals-dihedrals were stored in
                      // reverse order, as documented in "Parent.cpp"
      ddEntropy[index] =
          dihedralsEntropy1D[smaller] + dihedralsEntropy1D[bigger] - value;
    }
  }
}
...
void Entropy_Matrix::write_PAR_header() {
  ...
  outfile.write(
      (char *)&myversion,
      sizeof(int)); // first write the .par version number as an integer
  outfile.write(
      (char *)&double_prec,
      sizeof(int)); // then write an integer declaring if the trajectory was
                    // stored in double precision (-1 means unknown)
  outfile.write(
      (char *)&dummy,
      sizeof(int)); // write an integer containing the number of dihedrals
  outfile.write((char *)&numFrames,
                sizeof(int)); // and an integer containing the number of frames
                              // of the trajectory used for calculation
  outfile.write((char *)&bDens1D,
                sizeof(int)); // write the used number of bins for 1D histograms
  outfile.write((char *)&aDens1D, sizeof(int));
  outfile.write((char *)&dDens1D, sizeof(int));
  outfile.write((char *)&bDens,
                sizeof(int)); // and the used number of bins for 2D histograms
  ...
  for (int i = 0; i < dummy + 3; i++) { // for ever atom in the system
    ...
    outfile.write((char *)&(residueNumbers[i]),
  ...
  for (int i = 0; i < dummy + 3; i++) {
    outfile.write((char *)&(masses[i]),
                  sizeof(float)); // then write the whole massestor of the atoms
                                  // in the system in single precision (float)
  }
}
...
void Entropy_Matrix::read_PAR_header() {
  ...
  infile.read((char *)&version,
              sizeof(int)); // first read the .par version number as an integer
  infile.read((char *)&double_prec,
              sizeof(int)); // then read an integer declaring if the trajectory
                            // was stored in double precision (-1 means unknown)
  infile.read(
      (char *)&nDihedrals,
      sizeof(int)); // read an integer containing the number of dihedrals
  infile.read((char *)&numFrames,
              sizeof(int)); // and an integer containing the number of frames of
                            // the trajectory used for calculation
  infile.read((char *)&bDens1D,
              sizeof(int)); // read the used number of bins for 1D histograms
  infile.read((char *)&aDens1D, sizeof(int));
  infile.read((char *)&dDens1D, sizeof(int));
  infile.read((char *)&bDens,
              sizeof(int)); // and the used number of bins for 2D histograms
  ...
  if (version < 0) {
    My_Error my_error(
        (string("ERROR: FILE HEADER CORRUPTED. VERSION NUMBER (") +
         to_string(version) + string(") < 0! ABORTING."))
            .c_str());
    throw my_error;
  }
  if ((double_prec != 0) && (double_prec != 1) && (double_prec != -1)) {
    My_Error my_error(
        (string("ERROR: FILE HEADER CORRUPTED. DOUBLE PRECISION VALUE (") +
         to_string(double_prec) + string(") NEITHER 0, 1 NOR -1! ABORTING."))
            .c_str());
    throw my_error;
  }
  if (nDihedrals > 19997) {
    cerr << "WARNING: " << nDihedrals + 3
  ...
  if (nDihedrals < 0) {
    My_Error my_error(
        (string("ERROR: FILE HEADER CORRUPTED. NUMBER OF DIHEDRALS (") +
         to_string(nDihedrals) + string(") < 0! ABORTING."))
            .c_str());
    throw my_error;
  }
  if (numFrames < 0) {
    My_Error my_error(
        (string("ERROR: FILE HEADER CORRUPTED. NUMBER OF FRAMES (") +
         to_string(numFrames) + string(") < 0! ABORTING."))
            .c_str());
    throw my_error;
  }
  if (bDens1D < 1) {
    My_Error my_error(
        (string("ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR BONDS IN 1D (") +
         to_string(bDens1D) + string(") < 1! ABORTING."))
            .c_str());
    throw my_error;
  }
  if (aDens1D < 1) {
    My_Error my_error(
        (string(
             "ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR ANGLES IN 1D (") +
         to_string(aDens1D) + string(") < 1! ABORTING."))
            .c_str());
    throw my_error;
  }
  if (dDens1D < 1) {
    My_Error my_error((string("ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR "
                              "DIHEDRALS IN 1D (") +
                       to_string(dDens1D) + string(") < 1! ABORTING."))
                          .c_str());
    throw my_error;
  }
  if (bDens < 1) {
    My_Error my_error(
        (string("ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR BONDS IN 2D (") +
         to_string(bDens) + string(") < 1! ABORTING."))
            .c_str());
    throw my_error;
  }
  if (aDens < 1) {
    My_Error my_error(
        (string(
             "ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR ANGLES IN 2D (") +
         to_string(aDens) + string(") < 1! ABORTING."))
            .c_str());
    throw my_error;
  }
  if (dDens < 1) {
    My_Error my_error((string("ERROR: FILE HEADER CORRUPTED. NUMBER BINS FOR "
                              "DIHEDRALS IN 2D (") +
                       to_string(dDens) + string(") < 1! ABORTING."))
                          .c_str());
    throw my_error;
  }

  if (version >= 3) {
    for (unsigned int i = 0; i < nDihedrals + 3;
         i++) { // for every atom in the system
      infile.read(
          dummystring,
      ...
      belongsToMolecule.push_back(
          dummystring); // and the name of the molecule it belongs to
    }
  }
  ...
  for (unsigned int i = 0; i < nDihedrals; i++) { // then for all dihedrals
    ...
    infile.read((char *)&(dihedrals_top[i][4]),
                sizeof(int)); // an integer containing the type of the
                              // dihedral(physical=0,pseudo=1,improper=-1)
    if (!((dihedrals_top[i][4] == 1) || (dihedrals_top[i][4] == 0) ||
          (dihedrals_top[i][4] == -1))) {
      My_Error my_error(
          "ERROR: FILE HEADER CORRUPTED. DIHEDRAL TYPE NEITHER 0, 1 NOR -1 !");
      throw my_error;
    }
    infile.read((char *)&(dihedrals_top[i][5]),
                sizeof(int)); // and an integer containing the "parent"
                              // dihedral(for phaseangles, -1 if no "parent")
    if (dihedrals_top[i][5] < -1) {
      My_Error my_error("ERROR: FILE HEADER CORRUPTED. DIHEDRAL IS PHASEANGLE "
                        "OF A DIHEDRAL WITH NEGATIVE ID !");
      throw my_error;
    }
  ...
  for (unsigned int i = 0; i < nDihedrals + 3; i++) {
    masses.push_back(0);
    infile.read((char *)&(masses)[i],
                sizeof(float)); // then read the whole massestor of the atoms in
                                // the system in single precision (float)
  }
}

// to read the header of the binary .bat file
//~ int read_BAT_header(ifstream *infile,int *double_prec,int *numFrames,vector<
//vector <int> > *dihedrals_top, vector <float>  *masses, vector <string>
//*residues,vector <int> *residueNumbers,vector <string> *atomNames,vector
//<string> *belongsToMolecule) {
void Entropy_Matrix::read_BAT_header() {
  ...
  if ((double_prec != 0) && (double_prec != 1)) {
    ostringstream oss;
    oss << "ERROR: FILE HEADER CORRUPTED. DOUBLE PRECISION VALUE ("
        << double_prec << ") NEITHER 0 NOR 1!";
    My_Error my_error(oss.str());
    throw my_error;
  }
  if (nDihedrals > 19997) {
  ...
  if (nDihedrals < 0) {
    ostringstream oss;
    oss << "ERROR: FILE HEADER CORRUPTED. NUMBER OF DIHEDRALS (" << nDihedrals
        << ") < 0!";
    My_Error my_error(oss.str());
    throw my_error;
  }
  if (numFrames < 1) {
    ostringstream oss;
    oss << "ERROR: FILE HEADER CORRUPTED. NUMBER OF FRAMES (" << numFrames
        << ") < 1!";
    My_Error my_error(oss.str());
    throw my_error;
  }
...
unsigned int Entropy_Matrix::getNAngles() { return nAngles; }

unsigned int Entropy_Matrix::getNDihedrals() { return nDihedrals; }

// get the name of the residue of the atom with the according number
// (atomnumbers start at 1)
string Entropy_Matrix::getResidueName(unsigned int atomNumber) {
  if ((atomNumber < 1) || (atomNumber > residues.size())) {
    My_Error my_error(string("ERROR: REQUEST FOR THE RESIDUENAME OF AN ATOM "
                             "WITH AN INDEX WHICH IS OUT OF RANGE (") +
                      to_string(atomNumber) + string(").").c_str());
    throw my_error;
  }
...
int Entropy_Matrix::getResidueNumber(unsigned int atomNumber) {
  if ((atomNumber < 1) || (atomNumber > residues.size())) {
    My_Error my_error(string("ERROR: REQUEST FOR THE RESIDUENUMBER OF AN ATOM "
                             "WITH AN INDEX WHICH IS OUT OF RANGE (") +
                      to_string(atomNumber) + string(").").c_str());
    throw my_error;
  }
...
string Entropy_Matrix::getAtomName(unsigned int atomNumber) {
  if ((atomNumber < 1) || (atomNumber > residues.size())) {
    My_Error my_error(string("ERROR: REQUEST FOR THE NAME OF AN ATOM WITH AN "
...
string Entropy_Matrix::getMoleculeName(unsigned int atomNumber) {
  if ((atomNumber < 1) || (atomNumber > residues.size())) {
    My_Error my_error(
        string("ERROR: REQUEST FOR THE ASSOCIATED MOLECULENAME OF AN ATOM WITH "
               "AN INDEX WHICH IS OUT OF RANGE (") +
        to_string(atomNumber) + string(").").c_str());
    throw my_error;
  }
...
// returns the atomnumber of an atom in a given dihedral, e.g.
// getDihedralAtom(727,3) gives the number of the third atom in dihedral 727 (all
// indices start at 1)
int Entropy_Matrix::getDihedralAtom(unsigned int dihedralNumber,
                                    unsigned int atom) {
  if ((dihedralNumber < 1) || (dihedralNumber > dihedrals_top.size())) {
    My_Error my_error(
        string("ERROR: REQUEST FOR A DIHEDRAL NUMBER OUT OF RANGE (") +
        to_string(dihedralNumber) + string(").").c_str());
    throw my_error;
  }
  if ((atom < 1) || (atom > 4)) {
    My_Error my_error(
        string("ERROR: REQUEST FOR A DIHEDRAL WITH AN ATOM OUT OF RANGE (") +
        to_string(atom) + string(").").c_str());
    throw my_error;
  }
  return dihedrals_top[dihedralNumber - 1][atom - 1];
}

// returns the atomnumber of an atom in a given angle, e.g. getAngleAtom(575,1)
// gives the number of the first atom in angle 575 (all indices start at 1)
int Entropy_Matrix::getAngleAtom(unsigned int angleNumber, unsigned int atom) {
  if ((angleNumber < 1) || (angleNumber > dihedrals_top.size() + 1)) {
    My_Error my_error(
        string("ERROR: REQUEST FOR AN ANGLE NUMBER OUT OF RANGE (") +
        to_string(angleNumber) + string(").").c_str());
    throw my_error;
  }
  if ((atom < 1) || (atom > 3)) {
    My_Error my_error(
        string("ERROR: REQUEST FOR AN ANGLE WITH AN ATOM OUT OF RANGE (") +
        to_string(atom) + string(").").c_str());
    throw my_error;
  }
  if (angleNumber > 1) {
...
int Entropy_Matrix::getBondAtom(unsigned int bondNumber, unsigned int atom) {
  if ((bondNumber < 1) || (bondNumber > dihedrals_top.size() + 2)) {
    My_Error my_error(
        string("ERROR: REQUEST FOR A BOND NUMBER OUT OF RANGE (") +
        to_string(bondNumber) + string(").").c_str());
    throw my_error;
  }
  if ((atom < 1) || (atom > 2)) {
    My_Error my_error(
        string("ERROR: REQUEST FOR A BOND WITH AN ATOM OUT OF RANGE (") +
        to_string(atom) + string(").").c_str());
    throw my_error;
  }
  if (bondNumber > 2) {
...
void Entropy_Matrix::setPseudoZero() { // set mutual information terms involving
                                       // pseudo degrees of freedom zero;
  vector<unsigned int> pseudoBonds;
  vector<unsigned int> pseudoAngles;
  vector<unsigned int> pseudoDihedrals;

  if (belongsToMolecule[dihedrals_top[0][1] - 1] !=
      belongsToMolecule[dihedrals_top[0][0] - 1]) {
    pseudoBonds.push_back(1);
    pseudoAngles.push_back(1);
  }

  if (belongsToMolecule[dihedrals_top[0][2] - 1] !=
      belongsToMolecule[dihedrals_top[0][1] - 1]) {
    pseudoBonds.push_back(2);
    pseudoAngles.push_back(1);
  }

  for (unsigned int i = 0; i < nDihedrals; i++) {
    if (belongsToMolecule[dihedrals_top[i][3] - 1] !=
        belongsToMolecule[dihedrals_top[i][2] - 1]) {
      pseudoBonds.push_back(i + 3);
      pseudoAngles.push_back(i + 2);
      pseudoDihedrals.push_back(i + 1);
    }
    if (belongsToMolecule[dihedrals_top[i][2] - 1] !=
        belongsToMolecule[dihedrals_top[i][1] - 1]) {
      pseudoAngles.push_back(i + 2);
      pseudoDihedrals.push_back(i + 1);
    }
    if (belongsToMolecule[dihedrals_top[i][1] - 1] !=
        belongsToMolecule[dihedrals_top[i][0] - 1]) {
      pseudoDihedrals.push_back(i + 1);
    }
  }
