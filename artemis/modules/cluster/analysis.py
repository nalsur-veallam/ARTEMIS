...
def analyze_clustering(Clusters, noseq, out_path):
    ...
    for i in range(Clusters.NResidues):
        ...
        for j in range(i, Clusters.NResidues):
            if Clusters.clustering_labels[i] == Clusters.clustering_labels[j]:
                ...
                MIi_[Clusters.clustering_labels[i] - 1] += (
                    Clusters.map_[i, j] / quantity[Clusters.clustering_labels[i] - 1]
                )
            else:
                MIe[Clusters.clustering_labels[i] - 1] += Clusters.map_[i, j]
                MIe[Clusters.clustering_labels[j] - 1] += Clusters.map_[i, j]
                MIe_[Clusters.clustering_labels[i] - 1] += (
                    Clusters.map_[i, j] / quantity[Clusters.clustering_labels[i] - 1]
                )
                MIe_[Clusters.clustering_labels[j] - 1] += (
                    Clusters.map_[i, j] / quantity[Clusters.clustering_labels[j] - 1]
                )
                MIE[
                    Clusters.clustering_labels[i] - 1, Clusters.clustering_labels[j] - 1
                ] += (
                    Clusters.map_[i, j]
                    / quantity[Clusters.clustering_labels[i] - 1]
                    / quantity[Clusters.clustering_labels[j] - 1]
                )
                MIE[
                    Clusters.clustering_labels[j] - 1, Clusters.clustering_labels[i] - 1
                ] += (
                    Clusters.map_[i, j]
                    / quantity[Clusters.clustering_labels[i] - 1]
                    / quantity[Clusters.clustering_labels[j] - 1]
    ...
    CLUSTERING = pd.DataFrame(
        data=MIE[::-1, :], index=np.array(clNames)[::-1], columns=np.array(clNames)
    )
    ...
    INTENSITY = {
        "Intensity": np.array(intensity_act)
        / np.sqrt(sum(abs(np.array(intensity_act).flatten()) ** 2)),
        "Names": clNames,
    }
    axs[3] = sns.barplot(
        x="Names",
    ...
    INTENSITY = {
        "Intensity": np.array(intensity_act_)
        / np.sqrt(sum(abs(np.array(intensity_act_).flatten()) ** 2)),
        "Names": clNames,
    }
    axs[4] = sns.barplot(
        x="Names",
    ...
    INTENSITY = {
        "Intensity": np.array(intensity_all)
        / np.sqrt(sum(abs(np.array(intensity_all).flatten()) ** 2)),
        "Names": clNames,
    }
    axs[5] = sns.barplot(
        x="Names",
    ...
    INTENSITY = {
        "Intensity": np.array(intensity_all_)
        / np.sqrt(sum(abs(np.array(intensity_all_).flatten()) ** 2)),
        "Names": clNames,
    }
    axs[6] = sns.barplot(
        x="Names",
