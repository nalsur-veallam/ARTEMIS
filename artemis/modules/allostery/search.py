...
def search_allostery(
    Allostery, out_path, top, noseq, Zscore, Clustering=None, ClusterIdx=None
):
    ...
    if Clustering is None or ClusterIdx is None:
        ...
        for resid in Allostery.active_site:
            for i in range(
                int(max(0, resid - 1 - noseq)),
                int(min(Allostery.NResidues, resid + noseq)),
            ):
                mask[i] = 0
    else:
        ...
        for resid in Allostery.active_site:
            for i in range(
                max(0, resid - 1 - noseq), min(Allostery.NResidues, resid + noseq)
            ):
                mask[i] = 0
    ...
    if top is None and not Zscore:
        ...
        INTENSITY = {}
        INTENSITY["Intensity"] = np.array(intensity) / np.sqrt(
            sum(abs(np.array(intensity).flatten()) ** 2)
        )
        ...
        fig, axs = plt.subplots(
            figsize=(Allostery.NResidues * width, 20), constrained_layout=True
        )
        axs = sns.barplot(
            x="Residue",
            y="Intensity",
        ...
        plt.title(
            "Intensity of connectivity of residues with the active site", fontsize=40
        )
        try:
        ...
        plt.title(
            "Intensity of connectivity of residues with the active site zscore",
            fontsize=40,
        )
        try:
            fig.savefig(out_path)
        ...
        plt.title(
            "Intensity of connectivity of residues with the active site top "
            + str(top)
            + "%",
            fontsize=40,
        )
