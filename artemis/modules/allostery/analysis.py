...
def analyze_allostery(Allostery, out_path, top, noseq, Zscore):
    ...
    if Allostery.allosteric_site is None:
        print(
            "It is not possible to perform the analysis without an allosteric site. Interrupted.\n"
        )
    ...
    all_entropy = Allostery.map_[
        Allostery.allosteric_site - 1, Allostery.allosteric_site - 1
    ]
    act_Entropy = 0
    all_Entropy = 0
    connectivity = 0
    ...
    for i, res in enumerate(Allostery.allosteric_site):
        ...
        for j in range(Allostery.NResidues):
            if j + 1 not in Allostery.allosteric_site:
    ...
    print(
        "The mutual information of the active site with the remaining protein is",
        round(act_mi, 2),
    )
    print(
        "The mutual information of the allosteric site with the remaining protein is",
        round(all_mi, 2),
    )
    print(
        "The internal two-dimensional entropy of the active site is",
        round(act_Entropy, 2),
    )
    print(
        "The internal two-dimensional entropy of the allosteric site is",
        round(all_Entropy, 2),
    )
    print(
        "Allosteric connectivity between active and allosteric sites is",
        round(connectivity, 2),
        "\n\n",
    )
    ...
    axs[0].set_title(
        "Intensity of connectivity of residues\nfrom the active site", fontsize=30
    )
    ...
    axs[2].set_title(
        "Two-dimensional entropy of residues\nfrom the active site", fontsize=30
    )
    ...
    if Zscore:
        ...
        for i in range(Allostery.NResidues):
            new_names.append(
                Allostery.names[i] + " (" + str(Allostery.real_numbers[i]) + ")"
            )
        ...
        INTENSITY = INTENSITY.sort_values(
            by=["Intensity"], ascending=False, ignore_index=True
        )
        try:
        ...
        for i in range(NSites):
            if iters >= max_iters:
                cut = i
                print(
                    "Warning (zscore_top.py): The size of the active site is too big. Combinations of at most "
                    + str(cut)
                    + " remainders are investigated.\n"
                )
                break
        ...
        table = pd.DataFrame(
            columns=[
                "Combination",
                "Residues with zscore > 2",
                "Zscore > 2 Residues names",
                "Residues with zscore > 1.5",
                "Zscore > 1.5 Residues names",
                "Residues with zscore > 1",
                "Zscore > 1 Residues names",
                "Mean Intensity",
                "STD",
                "top 1 Residue",
                "top 2 Residue",
        ...
        for combinations in tqdm(Combinations, desc="Progress", total=cut):
            ...
            for active_site in tqdm(
                combinations,
                desc=f"ActSite size {leng} progress",
                total=comb(NSites, leng),
            ):
                intensity = []
                Allosteric = []
                for i in range(Allostery.NResidues):
                ...
                top1 = (
                    str(INTENSITY["Residue"][0])
                    + " ["
                    + str(np.round(INTENSITY["Intensity"][0], 2))
                    + "]"
                )
                top2 = (
                    str(INTENSITY["Residue"][1])
                    + " ["
                    + str(np.round(INTENSITY["Intensity"][1], 2))
                    + "]"
                )
                top3 = (
                    str(INTENSITY["Residue"][2])
                    + " ["
                    + str(np.round(INTENSITY["Intensity"][2], 2))
                    + "]"
                )
                ...
                for i in active_site:
                    act_s_name.append(
                        Allostery.names[i - 1]
                        + " ("
                        + str(Allostery.real_numbers[i - 1])
                        + ")"
                    )
                ...
                m2R = (
                    INTENSITY[INTENSITY["Intensity"] > 2]["Allosteric site"] == 1
                ).sum(axis=0)
                m15R = (
                    INTENSITY[INTENSITY["Intensity"] > 1.5]["Allosteric site"] == 1
                ).sum(axis=0)
                m1R = (
                    INTENSITY[INTENSITY["Intensity"] > 1]["Allosteric site"] == 1
                ).sum(axis=0)
                ...
                act_s1 = np.array(
                    INTENSITY1[INTENSITY1["Allosteric site"] == 1]["Residue"]
                )
                act_s15 = np.array(
                    INTENSITY15[INTENSITY15["Allosteric site"] == 1]["Residue"]
                )
                act_s2 = np.array(
                    INTENSITY2[INTENSITY2["Allosteric site"] == 1]["Residue"]
                )

                new_row = pd.Series(
                    {
                        "Combination": ", ".join(map(str, act_s_name)),
                        "Residues with zscore > 2": m2R,
                        "Zscore > 2 Residues names": ", ".join(map(str, act_s2)),
                        "Residues with zscore > 1.5": m15R,
                        "Zscore > 1.5 Residues names": ", ".join(map(str, act_s15)),
                        "Residues with zscore > 1": m1R,
                        "Zscore > 1 Residues names": ", ".join(map(str, act_s1)),
                        "Mean Intensity": np.round(mean, 2),
                        "STD": np.round(sigma, 2),
                        "top 1 Residue": top1,
                        "top 2 Residue": top2,
                        "top 3 Residue": top3,
                    }
        ...
        try:
            ...
            with pd.ExcelWriter(fname) as writer:
                ...
                overlap_all.to_excel(
                    writer, index=False, sheet_name="Allosteric sites overlap (lists)"
                )
                ovAmounts.to_excel(writer, index=False, sheet_name="overlap (amount)")
                ovAmounts_all.to_excel(
                    writer, index=False, sheet_name="Allosteric sites overlap (amount)"
                )
            print("\nFile", fname + " created\n")
        except Exception:
            print("\nError writing file", fname + "\n")
    ...
    if top is not None:
        ...
        for i in range(NSites):
            if iters >= max_iters:
                cut = i
                print(
                    "Warning (intensity_top.py): The size of the active site is too big. Combinations of at most "
                    + str(cut)
                    + " remainders are investigated.\n"
                )
                break
        ...
        for combinations in tqdm(Combinations, desc="Progress", total=cut):
            ...
            for active_site in tqdm(
                ...
                top10 = int(top / 100 * Allostery.NResidues)
                ...
                t10R = (INTENSITY.iloc[:top10]["Allosteric site"] == 1).sum(axis=0)
                INTENSITY = INTENSITY.iloc[:top10]
                act_s = np.array(
                    INTENSITY[INTENSITY["Allosteric site"] == 1]["Residue"]
                )
                ...
                new_row = pd.Series(
                    {
                        "Combination": ", ".join(map(str, act_s_name)),
                        "Top Residues": t10R,
                        "Top Residues names": ", ".join(map(str, act_s)),
                        "Mean Intensity": np.round(np.mean(INTENSITY["Intensity"]), 2),
                        "STD": np.round(np.std(INTENSITY["Intensity"]), 2),
                        "top 1 Residue": top1,
                        "top 2 Residue": top2,
                        "top 3 Residue": top3,
                    }
