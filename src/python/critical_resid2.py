...
for i in range(NResidues):
    if i + 1 in active_site or i + 1 in allosteric_site:
        ...
        for resid in active_site:
            connectiv_act += map_[resid - 1][i]
        for resid in allosteric_site:
            connectiv_all += map_[resid - 1][i]
        connectivity.append(connectiv_act*connectiv_all /(entropy[i] + const))
...
plt.title('Connectivity2 of residues with the active and allosteric sites for ' + name, fontsize=40)
fig.savefig(out_path + '_connectivity2.pdf')
...
with open(out_path + '_connectivity2.json', 'w') as outfile:
