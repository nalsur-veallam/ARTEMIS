...
def do_clustering(Clusters, out_path):
    ...
    if Clusters.reference_group is not None:
        map_ = Clusters.map_[
            :, Clusters.reference_group - 1
        ]  # Residual Distance Matrix
    ...
    if Clusters.restriction_group is None:
        clustering = AgglomerativeClustering(n_clusters=int(Clusters.NClusters)).fit(
            map_
        )
    ...
    for label in clustering.labels_:
        real_label = int(np.argwhere(unique == label)) + 1
        Clusters.clustering_labels.append(int(real_label))
    ...
    CLUSTERING = pd.DataFrame(
        data=MAP[::-1, :], index=np.array(labels)[::-1], columns=np.array(labels)
    )

    fig, axs = plt.subplots(figsize=(10, 10), constrained_layout=True)

    sns.heatmap(
        CLUSTERING, annot=False, cmap=clColors, linecolor="black", cbar=False
    )  # , linewidths = 0.1)

    plt.title(
        "Clustering matrix with " + str(Clusters.NClusters) + " clusters", fontsize=20
    )
    try:
...
def do_spectral_clustering(Clusters, out_path):
    ...
    clustering = SpectralClustering(
        n_clusters=int(Clusters.NClusters), affinity="precomputed"
    ).fit(Clusters.map_)
