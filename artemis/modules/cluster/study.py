...
def study_clustering(Clusters, min_of_clust, max_of_clust, out_path, spectral):
    ...
    for i in tqdm(range(min_of_clust, max_of_clust + 1)):
        ...
        external_mi = 0
        if spectral:
            clustering = SpectralClustering(n_clusters=i, affinity="precomputed").fit(
                map_
            )
        else:
        ...
        for j in range(i):
            ...
            for k, label in enumerate(clustering.labels_):
                if label == j:
                    centroid += map_[k] / L
    ...
    if not spectral:
        new_clustering = AgglomerativeClustering(
            distance_threshold=0, n_clusters=None
        ).fit(pd.DataFrame(data=map_, index=Clusters.names))

        fig, axs = plt.subplots(
            figsize=(12, heigh * Clusters.NResidues), constrained_layout=True
        )
        ...
        for i in range(Clusters.NResidues):
            labels.append(
                Clusters.names[i] + " (" + str(Clusters.real_numbers[i]) + ")"
            )

        plot_dendrogram(
            new_clustering, truncate_mode="level", orientation="right", labels=labels
        )
